...
## ADR-009: Стилизация — CSS Modules без UI-фреймворков

**Дата:** 2026-05-17
**Статус:** accepted

**Контекст:**
Sprint 0 имел технический долг — компоненты использовали inline `style={{...}}` объекты, что:
- Создавало новые объекты на каждом render (perf hit при 200+ tree nodes)
- Не позволяло использовать `:hover`, `:focus-visible`, `:active`, `[data-...]` псевдо-классы и атрибут-селекторы
- Затрудняло добавление transitions / animations (CSS быстрее JS)
- Конфликтовало с SVG-overlay (Sprint 1) — линии маппинга позиционируются по DOM-anchor'ам, для которых нужен стабильный класс-based DOM

Рассмотренные альтернативы для Sprint 0.5:
- **Tailwind CSS** — отвергнут: лишняя build-зависимость, тысячи utility classes в className, не вписывается в стиль Claude Design CSS-токенов с префиксом `--k-*`
- **styled-components / Emotion** — отвергнут: runtime CSS-in-JS добавит overhead, бандл +30-50 KB
- **Vanilla extracted CSS** (один глобальный stylesheet) — отвергнут: глобальный namespace, конфликты имён, неудобно при росте кодовой базы
- **CSS Modules** — выбран

**Решение:**
1. Каждый компонент имеет соседний `<Component>.module.css` файл
2. Import как `import styles from './Component.module.css'`, использование `<div className={styles.foo}>`
3. CSS-переменные из `src/styles/global.css` (= `konvey-design.css` от Claude Design) каталогизированы в `docs/DESIGN_TOKENS.md`
4. Runtime-вычисляемые стили (ширина прогресс-бара по %, динамическая позиция) — остаются как `style={{}}` — это допустимый паттерн
5. Утилитарные классы из `global.css` (`k-btn`, `k-input`, `k-mono`) можно использовать через `className="k-btn primary"` — для тривиальных случаев модуль не создаём

**Последствия:**
- Все компоненты Sprint 0 (10+ файлов) переписаны на CSS Modules в Sprint 0.5 Iter 2
- Появились proper hover/focus/active эффекты и transitions
- Bundle CSS scoped автоматически Vite'ом, нет глобальных конфликтов
- Перенос между темами/брендингами теперь — менять CSS-переменные в `global.css`, не трогая компоненты

---

## ADR-010: Иконки — inline SVG без библиотек

**Дата:** 2026-05-17
**Статус:** accepted

**Контекст:**
Sprint 0.5 потребовал ~30+ иконок (метаданные 1С, navigation, actions, status). Варианты:

- **lucide-react / react-icons / @heroicons/react** — отвергнуты: каждая добавляет 100-300 KB к bundle, лишняя зависимость
- **Иконочный шрифт** (Font Awesome) — отвергнут: blurry rendering на не-целых zoom levels, плохая accessibility
- **External SVG sprite** — отвергнут: лишний fetch, проблемы с CSP в Tauri WebView
- **Inline SVG в JSX** — выбран

**Решение:**
Создан `src/components/icons/Icon.tsx` с:
- 36 иконок (12 типов 1С метаданных, 6 navigation, 14 actions, 4 status)
- Все паттерны взяты из MIT-лицензированных наборов (Lucide ISC, Heroicons MIT) и встроены вручную
- API: `<Icon name="catalog" size={14} color="..." />`
- Helper: `iconNameForObjectType(objectType: string): IconName` и `iconNameForEdCategory(category)`

**Последствия:**
- Zero dependencies — bundle размером не страдает
- Все SVG используют `currentColor` для stroke — иконка наследует цвет родителя
- При необходимости добавить иконку — один файл, один enum entry, один SVG path
- Будущая замена на профессиональный icon-pack от дизайнера — переписать один компонент Icon.tsx, API остаётся

...
```powershell
git clone https://github.com/anymasoft/konvey.git
cd konvey

# 1. Проверить окружение (только проверка, ничего не ставит)
.\scripts\check-env.ps1

# 2. Установить недостающее по подсказкам скрипта.
#    Обычно нужно:
...
# 5. Перепроверить окружение - должно быть всё OK
.\scripts\check-env.ps1
```
...
- Загружает MSVC env (PATH/INCLUDE/LIB для `link.exe`) через `msvc-env.ps1`
- Создаёт sidecar placeholder если отсутствует (Tauri 2 требует existence-check)
- Стартует Vite (port 5173) + cargo run + spawn Python sidecar из venv
- Включает `RUST_LOG=info` для видимости логов sidecar
